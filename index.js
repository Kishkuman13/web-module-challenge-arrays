...
function getFlavorByIndex(arr, flvr){
    console.log(arr[flvr]);
}
...
function removeFlavorByName(arr, flvr){
    arr.splice(arr.indexOf(flvr), 1);
    console.log(arr);
}
...
function filterByWord(arr, flvr){
    let newarr = [];
    for ( let i = 0; i < arr.length; i++){
        if (arr[i].includes(flvr)){
            newarr.push(arr[i]);
        }
